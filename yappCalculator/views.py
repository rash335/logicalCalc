...
def allLogical(req):
    ...
    hex2 = req.POST.get('hex2');
    bits = int(req.POST.get('bits'));
    decop = req.POST.get('decop');
    binop = req.POST.get('binop');
    hexop = req.POST.get('hexop');

    if decop=='ORR': dec=functions.decimalORR(dec1,dec2,bits)
    if decop=='AND': dec=functions.decimalAND(dec1,dec2,bits)
    if decop=='XOR': dec=functions.decimalXOR(dec1,dec2,bits)
    if decop[0:3]=='NOT' and decop[-2]=='1': dec=functions.decimalNOT(dec1,bits)
    if decop[0:3]=='NOT' and decop[-2]=='2': dec=functions.decimalNOT(dec2,bits)
    decdec = dec;
    decbin = functions.decimal_binary_signed(dec,bits)
    dechex = functions.decimal_hexa_signed(dec,bits)

    if binop=='ORR': bin=functions.binaryORR(bin1,bin2,bits)
    if binop=='AND': bin=functions.binaryAND(bin1,bin2,bits)
    if binop=='XOR': bin=functions.binaryXOR(bin1,bin2,bits)
    if binop[0:3]=='NOT' and binop[-2]=='1': bin=functions.binaryNOT(bin1,bits)
    if binop[0:3]=='NOT' and binop[-2]=='2': bin=functions.binaryNOT(bin2,bits)
    bindec = functions.binary_decimal_signed(bin,bits)
    binbin = bin
    binhex = functions.binary_hexa_signed(bin,bits)

    if hexop=='ORR': hex=functions.hexaORR(hex1,hex2,bits)
    if hexop=='AND': hex=functions.hexaAND(hex1,hex2,bits)
    if hexop=='XOR': hex=functions.hexaXOR(hex1,hex2,bits)
    if hexop[0:3]=='NOT' and hexop[-2]=='1': hex=functions.hexaNOT(hex1,bits)
    if hexop[0:3]=='NOT' and hexop[-2]=='2': hex=functions.hexaNOT(hex2,bits)
    hexdec = functions.hexa_decimal_signed(hex,bits)
    hexbin = functions.hexa_binary_signed(hex,bits)
    hexhex = hex;

    data={'LogiDecDec':decdec,'LogiDecBin':decbin, 'LogiDecHex':dechex,
        'LogiBinDec':bindec,'LogiBinBin':binbin, 'LogiBinHex':binhex,
        'LogiHexDec':hexdec,'LogiHexBin':hexbin, 'LogiHexHex':hexhex,}
